...
class GLM_Model(torch.nn.Module):
    ...
    def add_spike_history(self, y):
        self.y = torch.flatten(torch.tensor(y, dtype=self.params.torch_d_type))
    ...
    def initialize_covariates(self):
        ...
        if self.params.inference_type == 'basis':
            ...
            self.optimizer = torch.optim.LBFGS([h], lr=1, history_size=100, max_iter=20, line_search_fn='strong_wolfe')
    ...
    def ml_closure(self, full_covariates, h):
        ...
        def closure():
            ...
            log_like = self.y.T @ full_covariates @ h - self.delta * torch.sum(torch.exp(full_covariates @ h)) - h[1:].t() @ h
            loss = -1 * log_like
            loss.backward()
